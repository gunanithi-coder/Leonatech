...
const GradientBadge = ({ label, color, icon, size = 'md' }) => {
  const sz = size === 'sm' ? { padding:'3px 9px', fontSize:'9px' } :
             size === 'lg' ? { padding:'6px 16px', fontSize:'12px' } :
...
const ServiceCard = ({ s, onClick }) => {
  ...
  return (
    <div className="expand-card" onClick={onClick} style={{ borderLeftColor: col }}>
      ...
      <div className="expand-front">
        ...
        <span
          ...
          style={{ background:`rgba(${hexToRgb(col)},.14)`, color:col }}
        >
          {s.category}
        </span>
      </div>
      ...
      {/* BACK */}
      <div className="expand-back" style={{ background:'var(--bg-mid)' }}>
        <div
...
const ServicesTabs = ({ go }) => {
  ...
  const visible = tab === 'All Services' ? SERVICES : SERVICES.filter(s => s.category === tab);
  return (
    <div>
      <div className="svc-tabs-wrap">
        <div className="svc-tabs">
          {SERVICE_TABS.map(c => (
            <button key={c} className={`svc-tab${tab === c ? ' on' : ''}`} onClick={() => setTab(c)}>
              {c}
            </button>
          ))}
        </div>
      </div>
      <div style={{ background:'var(--bg-dark)', padding:'0 60px 80px' }}>
        <div style={{ maxWidth:1400, margin:'0 auto', paddingTop:56 }}>
          <div className="svc-grid">
            {visible.map(s => <ServiceCard key={s.id} s={s} onClick={() => go('services')} />)}
          </div>
  ...
  return (
    <div className="page-transition">
      ...
      <section className="hero">
          ...
          <div className="hero-eyebrow fu">Drone · GIS · Geospatial Intelligence</div>
          ...
          <p className="hero-sub fu d2">
            High-accuracy drone surveying and geospatial solutions for infrastructure
            development, urban planning, and environmental assessment across 4 continents.
          </p>
          <div className="hero-actions fu d3">
            <button className="btn-primary" onClick={() => go('services')}>Explore Services</button>
            <button className="btn-ghost" onClick={() => go('projects')}>View Projects</button>
          </div>
        </div>
      </section>
      ...
      {/* ABOUT */}
      <section className="sec" style={{ background:'var(--bg-dark)' }}>
          ...
          <div className="about-grid">
            <div> 
              ...
              <div className="about-pts">
                {[
                  { i:'🚁', h:'Multi-Platform Fleet',   p:'Multi-coptor, Fixed Wing and VTOL systems with LiDAR, RGB and Oblique payloads.' },
                  { i:'📡', h:'Centimetre Accuracy',    p:'RTK and Static DGPS methods ensuring sub-5cm positional accuracy across all deliverables.' },
                  { i:'🌍', h:'Full-Spectrum GIS',      p:'Photogrammetry, LiDAR, bathymetrics, GPR, and satellite imagery in unified GIS workflows.' },
                ].map(pt => (
      ...
      {/* SERVICES */}
      <section style={{ background:'var(--bg-deep)', paddingTop:100 }}>
        <div style={{ maxWidth:1400, margin:'0 auto', padding:'0 60px 56px' }}>
          <SectionHeader
            centered
            label="What We Do"
      ...
      <section className="sec" style={{ background:'var(--bg-dark)' }}>
        <div className="sec-inner">
          ...
          <div className="ind-grid">
            {INDUSTRIES.map(i => (
              <div key={i.n} className="ind-card img-card"
      ...
      <div className="cta-sec">
        ...
        <div style={{ position:'relative' }}>
          <h2 className="cta-title">Ready to Map<br /><em>Your World?</em></h2>
          <p className="cta-sub">
            Partner with us for precise, dependable geospatial intelligence that powers
            your next infrastructure, urban planning, or environmental project.
          </p>
          <div className="cta-btns">
            <button className="btn-primary" onClick={() => go('contact')}>Contact Us Today</button>
            <button className="btn-ghost"   onClick={() => go('services')}>Browse All Services</button>
          </div>
        </div>
      </div>
    </div>
